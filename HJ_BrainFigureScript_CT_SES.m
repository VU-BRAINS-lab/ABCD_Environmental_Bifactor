...
CT_SES_Sig = csvread([ProjectFolder '/CT_SES.csv'], 1);
ind_lh = find(CT_SES_Sig(:, 1) < 2000);
CT_SES_Sig_lh = CT_SES_Sig(ind_lh, :);
ind_rh = find(CT_SES_Sig(:, 1) > 2000);
CT_SES_Sig_rh = CT_SES_Sig(ind_rh, :);
% lh
Vertex_AllLabel_InAtlas_lh = Name_lh.table(2:end, 5);
VertexStatistical_lh = zeros(size(VertexLabel_lh));
Sig_ID = CT_SES_Sig_lh(:, 1);

...
CT_Urban_Sig = csvread([ProjectFolder '/CT_Urban.csv'], 1);
ind_lh = find(CT_Urban_Sig(:, 1) < 2000);
CT_Urban_Sig_lh = CT_Urban_Sig(ind_lh, :);
ind_rh = find(CT_Urban_Sig(:, 1) > 2000);
CT_Urban_Sig_rh = CT_Urban_Sig(ind_rh, :);
% lh
Vertex_AllLabel_InAtlas_lh = Name_lh.table(2:end, 5);
VertexStatistical_lh = zeros(size(VertexLabel_lh));
Sig_ID = CT_Urban_Sig_lh(:, 1);

...
Vol_Urban_Sig = csvread([ProjectFolder '/Vol_Urban.csv'], 1);
ind_lh = find(Vol_Urban_Sig(:, 1) < 2000);
Vol_Urban_Sig_lh = Vol_Urban_Sig(ind_lh, :);
ind_rh = find(Vol_Urban_Sig(:, 1) > 2000);
Vol_Urban_Sig_rh = Vol_Urban_Sig(ind_rh, :);
% lh
Vertex_AllLabel_InAtlas_lh = Name_lh.table(2:end, 5);
VertexStatistical_lh = zeros(size(VertexLabel_lh));
Sig_ID = Vol_Urban_Sig_lh(:, 1);
